...
mutable struct VkFFTPlan{T <: Union{ComplexF32, ComplexF64}} <: AbstractFFTs.Plan{T}
    ...
    function VkFFTPlan(app::Ptr{Cvoid}, config::Ptr{Cvoid}, direction::Int32, dims::NTuple{N, Int} where N, region, buffer_size::AbstractArray{Int}, type::DataType, is_inplace::Bool, is_normalized::Bool)
        plan = new{type}(app, config, direction, dims, region, buffer_size, is_inplace, is_normalized)
...
function _new_config(fft_dim::Int64, buffer_dim::Int64, dims::AbstractArray{Int64}, omit_dims::AbstractArray{Int64}, num_batches::Int64, coalesced_memory::Int64, aim_threads::Int64, num_shared_banks::Int64, forward::Bool, double_precision::Bool, inplace::Bool, normalize::Bool)
    return @ccall libvkfft.new_config(fft_dim::Culonglong, buffer_dim::Int64, dims::Ptr{Culonglong}, omit_dims::Ptr{Culonglong}, num_batches::Culonglong, coalesced_memory::Culonglong, aim_threads::Culonglong, num_shared_banks::Culonglong, forward::Cuchar, double_precision::Cuchar, inplace::Cuchar, normalize::Cuchar)::Ptr{Cvoid}
end
...
function _make_plan(x::CuArray{T}, region, forward::Bool, inplace::Bool, normalize::Bool; coalesced_memory::Int=0, aim_threads::Int=0, num_shared_banks::Int=0) where T <: Union{ComplexF32, ComplexF64}
    ...
    config = _new_config(num_fft_dim, num_buffer_dim, dim_sizes, omit_dims, num_batches, coalesced_memory, aim_threads, num_shared_banks, forward, double_precision, inplace, normalize)
    if config == C_NULL
    ...
    return VkFFTPlan(app, config, direction, size(x), region, dim_sizes, eltype(x), inplace, normalize)
end
...
function _get_tuned_params(x::CuArray{T}, region, forward::Bool, inplace::Bool, normalize::Bool; num_ffts::Int=1000) where T <: Union{ComplexF32, ComplexF64}
    ...
    for (coalesced_memory, aim_threads) in params
        plan = _make_plan(x, region, forward, inplace, normalize; coalesced_memory=coalesced_memory, aim_threads=aim_threads)
...
function _make_tuned_plan(x::CuArray{T}, region, forward::Bool, inplace::Bool, normalize::Bool; tune::Bool=false) where T <: Union{ComplexF32, ComplexF64}
    if tune
        coalesced_memory, aim_threads = _get_tuned_params(x, region, forward, inplace, normalize)
    else
        coalesced_memory, aim_threads = (0, 0)
    end
    return _make_plan(x, region, forward, inplace, normalize; coalesced_memory=coalesced_memory, aim_threads=aim_threads)
end

Base.size(plan::VkFFTPlan) = tuple(plan.buffer_size...)
Base.eltype(plan::VkFFTPlan) = typeof(plan).parameters[1]
AbstractFFTs.plan_fft(x::CuArray{T}, region; tune::Bool=false) where T <: Union{ComplexF32, ComplexF64} = _make_tuned_plan(x, region, true, false, false; tune=tune)
AbstractFFTs.plan_fft!(x::CuArray{T}, region; tune::Bool=false) where T <: Union{ComplexF32, ComplexF64} = _make_tuned_plan(x, region, true, true, false; tune=tune)
AbstractFFTs.plan_bfft(x::CuArray{T}, region; tune::Bool=false) where T <: Union{ComplexF32, ComplexF64} = _make_tuned_plan(x, region, false, false, false; tune=tune)
AbstractFFTs.plan_bfft!(x::CuArray{T}, region; tune::Bool=false) where T <: Union{ComplexF32, ComplexF64} = _make_tuned_plan(x, region, false, true, false; tune=tune)
...
function AbstractFFTs.plan_inv(plan::VkFFTPlan)
    ...
    invplan = _make_plan(x, region, !forward, plan.is_inplace, false)
